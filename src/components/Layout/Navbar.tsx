    ...
    <>
      <nav className="h-[90px] flex items-center bg-transparent">
        <div className="w-[96%] mx-auto flex items-center justify-between text-gray-800">
          ...
          <div className="w-[250px] items-center justify-end gap-5 font-medium text-gray-800 hidden md:flex">
            <RefLink className="hidden font-semibold text-sm sm:flex" target="_blank" href={"https://app.toolbird.io/auth/login"}>
              Log in
            </RefLink>
            <button
          ...
          <div className="flex md:hidden cursor-pointer">
            {menuClicked ? <X onClick={() => setMenuClicked(false)} />:<Menu onClick={() => setMenuClicked(true)} />}
          </div>
        </div>
      </nav>
      {menuClicked && 
      <Reveal delayTime={0}>
        <div className="bg-white text-left flex flex-col justify-between gap-4 font-semibold text-3xl w-[90%] mx-auto py-8 h-[80vh] md:hidden">
          ...
          <RefLink className="bg-transparent border-[3px] w-full border-main text-sm font-semibold text-main px-6 py-2 rounded-full hover:bg-main hover:text-white transition-all" target="_blank" href={"https://app.toolbird.io/auth/login"}>
              Log in
            </RefLink>
            <button
              onClick={() => NiceModal.show(JoinWaitlistModal)}
